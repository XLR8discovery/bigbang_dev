...
func buildServices(services []runtime.Service) error {
	...
	for _, service := range services {
		for _, mount := range service.GetVolumes() {
			if mount.MountType == "bind" && filepath.Dir(mount.Target) == filepath.Clean("/var/lib/bigbang/go/bin") {
				var codeSource string
				var foundCodeSource bool
				serviceCodeSource, foundCodeSource := service.GetENV()["BIGBANG_UP_LOCAL_BINARY_SOURCE"]
				if foundCodeSource {
					codeSource = *serviceCodeSource
				} else {
					codeSource, foundCodeSource = os.LookupEnv("BIGBANG_UP_LOCAL_" + service.ID().Name)
				}
...
func runCommand(cmd *exec.Cmd, runPath string) error {
	...
	fmt.Println("*** Bigbang-Up Running " + strings.Join(cmd.Args, " ") + " from " + runPath + " ***")
	cmd.Dir = runPath
	err := cmd.Start()
	if err != nil {
		return err
