...
import (
	...
	"xlr8d.io/bigbang-up/cmd"
	"xlr8d.io/bigbang-up/pkg/common"
...
var frontendDict = map[string]mount{
	"web.satellite": {
		...
		targetPath: "/var/lib/bigbang/bigbang/web/satellite",
	},
	"web.multinode": {
		service:    []string{"storagenode"},
		targetPath: "/var/lib/bigbang/web/multinode",
	},
	"web.storagenode": {
		service:    []string{"storagenode"},
		targetPath: "/var/lib/bigbang/web/storagenode",
	},
	"admin.ui": {
		service:    []string{"satellite-admin"},
		targetPath: "/var/lib/bigbang/bigbang/satellite/admin/ui",
	},
}
...
func localBinCmd() *cobra.Command {
	mountCmd := &cobra.Command{
		...
		RunE: cmd.ExecuteBigbangUP(func(st recipe.Stack, rt runtime.Runtime, selector []string) error {
...
func mountBinaries(composeService *types.ServiceConfig) error {
	...
	target := filepath.ToSlash(filepath.Join("/var/lib/bigbang/go/bin", execName))
