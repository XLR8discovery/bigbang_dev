...
var RootCmd = &cobra.Command{
	Use:   "bigbang-up",
	Args:  cobra.NoArgs,
	Short: "A golang wrapper for creating customized Bigbang environment with many components",
	Long: `bigbang-up can be used to create containerized or standalone runtime environment,
leverages existing binaries, or even references code to be built in docker and create images.

For example:

bigbang-up build remote gerrit 5826`,
}

// Execute is the execution of the top level bigbang-up command.
func Execute() {
	err := RootCmd.Execute()
	if err != nil {
		log.Fatalf("%++v", err)
...
// ExecuteBigbangUP can execute any operation with loaded stack/runtime and write back the results.
func ExecuteBigbangUP(exec func(stack recipe.Stack, rt runtime.Runtime, args []string) error) func(cmd *cobra.Command, args []string) error {
