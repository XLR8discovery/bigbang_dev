...
import (
	...
	pkg "xlr8d.io/bigbang-up/pkg"
	"xlr8d.io/bigbang/web/satellite/wasm/consolewasm"
)
...
type Credentials struct {
	BigbangUser     string `json:"email"`
	BigbangPassword string `json:"password"`
...
func credentialsCmd() *cobra.Command {
	...
	pflags.StringVarP(&credentials.BigbangUser, "email", "m", "test@xlr8d.io", "The email of the test user to use/create")
	pflags.StringVarP(&satelliteHost, "satellite", "s", "localhost:7777", "The host and port of of the satellite api to connect. Defaults to localhost or BIGBANG_DOCKER_HOST if set.")
	pflags.StringVarP(&consoleHost, "console", "c", "localhost:10000", "The host and port of of the satellite api console to connect. Defaults to localhost or BIGBANG_DOCKER_HOST if set.")
	pflags.StringVarP(&authServiceHost, "authservice", "a", "http://localhost:8888", "Host of the auth service. Defaults to localhost or BIGBANG_DOCKER_HOST if set.")
...
func attemptUpdateDockerHost() error {
	dockerHost := os.Getenv("BIGBANG_DOCKER_HOST")
	if dockerHost != "" {
...
func generateCredentials(ctx context.Context) error {
	...
	consoleEndpoint := pkg.NewConsoleEndpoints(consoleHost, credentials.BigbangUser)
...
func registerS3Credentials(ctx context.Context) error {
	if _, err := os.Stat("docker-compose.yaml"); err == nil {
		fmt.Println("Looks like you have a docker-compose.yaml. I suspect you execute this command from the host, not from the container. Please note that S3 compatible access Grant should use the container network host (satellite-api). Therefore it should be executed from the container. (docker-compose exec satellite-api bigbang-up credentials -s3)")
	}
	var err error
...
func persistCredentials() error {
	credentials.BigbangPassword = password
...
func printCredentials() error {
	if !export {
		...
		fmt.Printf("export BIGBANG_USER=%s\n", credentials.BigbangUser)
		fmt.Printf("export BIGBANG_USER_PASSWORD=%s\n", password)
		fmt.Printf("export BIGBANG_PROJECT_ID=%s\n", credentials.ProjectID)
		fmt.Printf("export BIGBANG_SESSION_COOKIE=Cookie: _tokenKey=%s\n", credentials.Cookie)
		fmt.Printf("export BIGBANG_API_KEY=%s\n", credentials.ApiKey)

		fmt.Printf("export BIGBANG_ENCRYPTION_SECRET=%s\n", secret)
		fmt.Printf("export BIGBANG_ACCESS=%s\n", credentials.Grant)
		fmt.Printf("export UPLINK_ACCESS=%s\n", credentials.Grant)

		fmt.Printf("export AWS_ACCESS_KEY_ID=%s\n", credentials.AccessKey)
		fmt.Printf("export AWS_SECRET_ACCESS_KEY=%s\n", credentials.SecretKey)
		fmt.Printf("export BIGBANG_GATEWAY=%s\n", credentials.Endpoint)
	}
	return nil
}
