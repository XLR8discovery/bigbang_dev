...
func healthCmd() *cobra.Command {
	...
	cmd.PersistentFlags().StringVarP(&host, "host", "", "localhost", "host/ip address to use for health check. Defaults to localhost or BIGBANG_DOCKER_HOST if set.")
...
func checkHealthCockroachPostgres(table string, records int) error {
	prevCount := -1
	defaultHost := os.Getenv("BIGBANG_DOCKER_HOST")
